		...
		queue.forEach((item) => {
			if (item.status === SendSatuts.pending) {
				info({
					...
					title:
						item.message ||
						`${item.call.method.method}-${item.call.method.section}`,
				});
			}
			...
			if (item.status === SendSatuts.failed) {
				error({
					...
					title:
						item.message ||
						`${item.call.method.method}-${item.call.method.section} Failed`,
				});
			}
			...
			if (item.status === SendSatuts.success) {
				success({
					...
					title:
						item.message ||
						`${item.call.method.method}-${item.call.method.section} Success`,
				});
			}
		});
