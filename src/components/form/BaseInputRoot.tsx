...
function getRootClassName(size: Size, error: boolean, focuse: boolean, customClassName: string) {
  const base = 'flex px-16 py-12 border rounded-[12px] text-left cursor-pointer items-center transition-all';
...
export const BaseInputRoot: FC<BaseInputRootProps> = React.memo(
  ({ size = 'md', children, error, render, className = '', focuse = false }) => {
    const finalRender = render ? render : typeof children === 'function' ? children : () => children;

    const rootClassName = getRootClassName(size, error, focuse, className);

    return <div className={rootClassName}>{finalRender({ focuse })}</div>;
  }
);
