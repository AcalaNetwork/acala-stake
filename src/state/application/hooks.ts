...
export const useSetSelectedAddress = () => {
	...
	return useCallback(
		(address: string) => dispatch(setSelectedAddress({ value: address })),
		[dispatch]
	);
};

export const useSelectedAddress = () =>
	useSelector((state: StoreState) => state.application.selectedAddress);

// modal
export const useModalVisible = (type: ModalType) =>
	useSelector(
		(state: StoreState) => state.application.modal[type]?.visible || false
	);
