...
export const useSwapCall = (_configs: configs) => {
  ...
  const [call, fee] = useExtrinsic({
    ...
    section: 'dex',
    method: configs.mode === 'EXACT_INPUT' ? 'swapWithExactSupply' : 'swapWithExactTarget',
  ...
  const message = useMemo(() => {
    ...
    return `Swap ${getTokenName(result.output.token)} from ${getTokenName(result.input.token)}`;
  }, [result]);
