    ...
    <div className="gap-100 flex flex-center">
      <div className="flex-1">
        <StakeItem
          apy={acalaHomaAPY}
          className="flex-1"
          desc={`Stake ${getTokenName(acalaHoma.consts.stakingToken)} to receive daily rewards and retain control of your staked ${getTokenName(acalaHoma?.consts.stakingToken)}s in ${getTokenName(acalaHoma.consts.liquidToken)}.`}
      ...
      <div className="flex-1">
        <StakeItem
          apy={karuraHomaAPY}
          className="flex-1"
          desc={`Stake ${getTokenName(karuraHoma.consts.stakingToken)} to receive daily rewards and retain control of your staked ${getTokenName(karuraHoma.consts.stakingToken)}s in ${getTokenName(karuraHoma?.consts.liquidToken)}.`}
  ...
  return (
    ...
    >
      ...
      <div className="tracking-[0.02em] text-center w-[322px]">
        ...
        <Spacing h={19} />
        <div className="text-14 leading-21 font-medium text-grey-3">
          {desc}
        </div>
      </div>
      <Spacing h={29} />
      <div className="flex w-full h-99 border border-d6d3de rounded-[24px] py-11 text-28">
        ...
        <div className="flex-1 flex-col flex justify-around items-center">
          ...
          {token.toString() === "DOT" && (
            <span className="text-e40c5b font-medium text-11 leading-13">
              +10% APR ACA airdrop
            </span>
          )}
        </div>
