  ...
  const unstakeColumns = [
    {
      ...
      accessor: "request",
      Cell: (props: any) => (
        <div className="flex items-center text-primary font-medium text-14">
          {props.value}
        </div>
      ),
    },
    {
      ...
      Cell: (props: any) => (
        <div className="text-12 font-medium">
          {props.value === "WAITING" ? (
            <div className="text-[#ff8800] border border-[#ff8800] bg-[#ff8800] bg-opacity-10 w-120 h-32 rounded-16 flex flex-center">{props.value}</div>
          ) : (
            <div className="text-[#61c288] border border-[#61c288] bg-[#61c288] bg-opacity-10 w-120 h-32 rounded-16 flex flex-center">{props.value}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      accessor: "duration",
      Cell: (props: any) => (
        <div className="flex items-center text-primary text-14 font-medium">
          {props.value} Days
        </div>
      ),
    },
  ...
  const rewardColumns = [
    {
      ...
      accessor: "amount",
      Cell: (props: any) => (
        <div className="flex flex-center text-494853 text-14">
          {props.value}
        </div>
      ),
    },
      ...
      <div className="flex flex-center gap-40 text-16 leading-[32px] font-semibold cursor-pointer">
        <div
          className={`border-b-2 ${
            tab === 0 ? "text-primary border-primary" : " border-transparent"
          }`}
          onClick={() => setTab(0)}
        >
          Unstake Requests
        ...
        <div
          className={`border-b-2 ${
            tab === 1 ? "text-primary border-primary" : " border-transparent"
          }`}
          onClick={() => setTab(1)}
        >
          Reward History
